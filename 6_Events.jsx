...
// In web development, When an event (like a click) happens on an element, it
// first runs the handlers on that specific element, then on its parent, then
// all the way up to the very top (the document).
// This happens only when there is a onClick event defined on each of those
...
// Instead of adding an onClick to 100 different list items (<li>), you add one listener to
// the parent (<ul>).
// When any list item is clicked, the event bubbles up to the parent, and you can
// handle it there.
...
// When you click an element, the event actually travels in three distinct
// stages. It doesn't just "appear" at the button; it travels from the root down,
// and then back up.
...
// The event "bubbles" up from the target element, back through all the parents,
// until it hits the window again.
// Note: This is the phase we usually care about and where onClick listeners in
// React fire by default.

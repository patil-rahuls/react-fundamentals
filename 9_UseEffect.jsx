...
// The "useEffect" hook in React allows developers to
// perform 'side effects' such as data fetching, setting
// up/clearing up timers, in the lifecycle of any functional
// components.

// The "useEffect" hook takes two arguments -
//      1. a function(side-effect) and
//      2. an array of dependencies that determines when
//         the effect should be run.

// In the following example, we have -
// A random number generator button and
// an h1 tag showing the random number.
...
export const RandomNumberApp = () => {
  ...
  useEffect(() => {
    ...
    // Scenario 4 with dependencies are not provided. []
    useEffect(() => {
      console.log('Creating socket connection...');
      // Return a clean-up function.
      return () => {
    ...
    //   or set up a timer, cleanup cancels or removes these
    //   when they're no longer needed.
    // Avoid memory leaks:
    //   When components unmount, cleanup functions prevent
    //   lingering subscriptions, timers, or event listeners
    //   that could cause memory leaks or unexpected behavior.
    // Manage subscriptions or event listeners:
    //   Add event listeners (like window resize or scroll)
    //   or subscribe to data sources, and clean them up to
    //   prevent multiple subscriptions.
    // Clear timers or intervals:
    //   If you use setTimeout or setInterval, cleanup
    //   clears them to avoid executing code after the
    //   component is gone.
